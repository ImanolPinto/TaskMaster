...
namespace TaskMaster.Test.MainViewModelTests
{
    ...
    public class PauseCommandTests
    {
        ...
        public void Given_a_playing_task_that_can_be_paused_in_the_TaskItemPlayer_when_another_task_is_selected_then_the_Pause_command_can_be_handled()
        {
            ...
            // When
            sut.SelectedTask = sut.ActiveTaskList[1];
        ...
        public void Given_a_pausable_task_when_it_is_not_selected_then_it_can_be_paused()
        {
            ...
            taskPlayerMock.Setup(x => x.CanPause(mainViewModel.ActiveTaskList[0])).Returns(true);
        ...
        [Test]
        public void Given_a_pausable_task_in_the_active_task_list_and_another_task_is_selected_when_pause_command_is_received_then_pause_is_called_for_the_first_task()
        {
            // Given
            Mock<ITaskPlayer> taskPlayerMock = null;
            var mainViewModel = MVMHelpers.SutWithTaskListAndPlayerMock(out taskPlayerMock);
            mainViewModel.ActiveTaskList[0] = new TaskItemBuilder(Guid.NewGuid()).Build();
            taskPlayerMock.Setup(x => x.CanPause(mainViewModel.ActiveTaskList[0])).Returns(true);
            mainViewModel.SelectedTask = mainViewModel.ActiveTaskList[1];
            ...
            // Then
            taskPlayerMock.Verify(x => x.Pause(mainViewModel.ActiveTaskList[0]), Times.Once());
        }

        [Test]
        public void Given_a_task_that_has_paused_state_in_the_active_task_list_When_another_playable_task_from_the_list_is_played_then_all_the_other_tasks_are_asigned_null_state()
        {
            // Given
            Mock<ITaskPlayer> taskPlayerMock = null;
            var mainViewModel = MVMHelpers.SutWithTaskListAndPlayerMock(out taskPlayerMock);
            mainViewModel.ActiveTaskList[0].PlayingState = PlayingState.Paused;
            var playableTask = mainViewModel.ActiveTaskList[1];
            ...
            // Then
            Assert.IsFalse(mainViewModel.ActiveTaskList.ToList().Any(x => x.PlayingState != null && x.Id != mainViewModel.SelectedTask.Id));
        }
